...
public class BankImpl implements Bank {
    ...
    private AcquiredAccount acquire(int index, Op op) {
        ...
        while (true) {
            ...
            if (account instanceof AcquiredAccount) {
                ...
                AcquiredAccount acquiredAccount = new AcquiredAccount(account.amount, op);
                if (!op.completed && accounts.compareAndSet(index, account, acquiredAccount)) {
                    return acquiredAccount;
                }
            }
        }
    ...
    private class TransferOp extends Op {
        ...
        void invokeOperation() {
             ...
             * }
             */
            final AcquiredAccount from;
            final AcquiredAccount to;
            ...
            if (to != null && from != null) {
                if (amount > from.amount) {
                    ...
                    // (account.amount +/- amount) is constant, so we don't care about data race here
                    from.newAmount = from.amount - amount;
                    to.newAmount = to.amount + amount;
                }
                completed = true; // volatile write to completed field _after_ the new amounts are calculated
            }
//            if (fromIndex < toIndex) {
